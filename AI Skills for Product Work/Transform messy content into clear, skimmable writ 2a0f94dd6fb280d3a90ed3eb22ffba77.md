# Transform messy content into clear, skimmable writing (based on Wes Kao’s course)
...
* Turn bullets → sentences where logic needs to be explicit.
* Check flow: each paragraph should begin with a signpost that previews its role.
* MOO check: add one sentence that answers the most obvious objection.

## Edge Cases

* If CONTENT is very short, still add In short, and As a next step, lines.
* If evidence is missing, write Because with first-principles reasoning and add For example with a plausible illustration, clearly labeled.
...
* Verbs over nouns; active voice.

## Output Format

* Plain text with numbered sections per the template.
* Do not include this instruction block in the output.
